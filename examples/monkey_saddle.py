...
factor_edge_length = 1.1 # edge length factor

residual_force_goal_weight = 100.0 # weight for residual force goal in optimisation
edge_length_goal_weight = 1.0 # weight for edge length goal in optimisation
...
for edge in network0.edges():
	...
	goals.append(LengthGoal(edge, factor_edge_length * current_length, weight=edge_length_goal_weight))

# reaction forces
for key in network0.nodes_supports():
	step = steps[key]
	reaction = (1 - step / max_step) ** r_exp * (rmax - rmin) + rmin
	goals.append(ResidualForceGoal(key, reaction, weight=residual_force_goal_weight))

# solve/optimise
network = constrained_fdm(network0, optimizer=SLSQP(), loss=squared_loss, goals=goals, bounds=(qmin, qmax), maxiter=maxiter, tol=tol)
