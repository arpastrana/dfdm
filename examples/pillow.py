...
angle_vector = [0.0, 0.0, 1.0]  # reference vector to compute angle in constraint
...
if add_normal_angle_constraint:
    msg = "Normal angle constraint w.r.t vertical between {} and {} degrees"
    print(msg.format(round(angle_min / pi * 180, 1), round(angle_max / pi * 180, 1)))
    for key in network.nodes():
        if not mesh.is_vertex_on_boundary(key):
            polygon = mesh.vertex_neighbors(key, ordered=True)
            constraint = NodeNormalAngleConstraint(key,
                                                   polygon,
                                                   [0.0, 0.0, 1.0],
                                                   bound_low=angle_min,
                                                   bound_up=angle_max)
